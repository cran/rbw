...
\title{\pkg{rbw}: An R package for Constructing Residual Balancing Weights}
...
RBW is also advantageous from a computational perspective. Because its
...
\item
  ...
  mediator variable.
\item
  \code{zmodels} is a list of fited \code{lm} or \code{glm} objects for
...
We now illustrate the functions \code{rbwPanel()}, \code{rbwPoint()},
and \code{rbwMed()} with data sets \code{advertisement}, \code{peace},
...
\code{rbw::rbwPoint()} will construct the residual balancing weights by
...
sampling weights, \code{rbw::rbwPoint()} will set a vector of ones with
...
Let \(A_{t}\) represent our continuous treatment, the proportion of
...
on the treatment.\footnote{The negative advertising data set spans five
  ...
  time-varying covariates on the treatment may set \code{future > 0}, up
  to \code{future = 4}.} Since our data do not include sampling weights,
